 ...
 * Version: Maritime Documents Universal Processor v2.0
...
function handleUploadFile(data) {
  try {
    console.log("📁 Maritime document upload action received");
    ...
    // Simulate successful upload with document type context
    ...
    console.error("❌ Maritime document upload error: " + error.toString());
    return createJsonResponse(false, "Maritime document upload failed: " + error.toString());
  }
}
...
function createStructuredSummary(text, documentType, documentInfo) {
  try {
    ...
    // Key fields for this document type
    ...
    if (cleanText.length > 1000) {
      summary += "Full Content: " + cleanText.substring(0, 1000) + "... [truncated]\n\n";
    } else {
      summary += "Full Content: " + cleanText + "\n\n";
    }
    
    // Try to identify patterns based on document type
    ...
    summary += "\n" + getDocumentSpecificAnalysis(cleanText, documentType, documentInfo);
