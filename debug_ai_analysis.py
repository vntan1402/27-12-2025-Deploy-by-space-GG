...
def test_ai_analysis_directly():
    ...
    # Login first
    response = requests.post('https://continue-session.preview.emergentagent.com/api/auth/login', 
    ...
    try:
        with open(temp_file_path, 'rb') as f:
            ...
            response = requests.post(
                'https://continue-session.preview.emergentagent.com/api/analyze-ship-certificate',
                files=files, 
                headers=headers,
                timeout=60
            )
