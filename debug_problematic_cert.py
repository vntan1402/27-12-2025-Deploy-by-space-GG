#!/usr/bin/env python3
"""
Debug Problematic Certificate - Test the certificate that used full name instead of abbreviation
"""
...
try:
    ...
    BACKEND_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://vessel-cert.preview.emergentagent.com') + '/api'
...
def debug_problematic_certificate():
    """Debug the certificate that used full name instead of abbreviation"""
    token = authenticate()
    if not token:
    ...
    # Test with the problematic LL certificate
    cert_id = "3d773205-9f70-4085-b2d5-60858bbdae35"  # This one used full name
    
    print(f"🔍 Debugging PROBLEMATIC certificate: {cert_id}")
    ...
    if cert_response.status_code == 200:
        ...
        # Check if abbreviation is actually present
        cert_abbreviation = cert.get('cert_abbreviation')
        if cert_abbreviation and cert_abbreviation.strip() and cert_abbreviation != 'N/A':
            print(f"   ✅ Certificate has abbreviation: '{cert_abbreviation}'")
        else:
            print(f"   ❌ Certificate missing abbreviation: '{cert_abbreviation}'")
    else:
    ...
    print(f"\n🔍 LOGIC CHECK:")
    print(f"   cert_abbreviation exists: {bool(cert_abbreviation)}")
    print(f"   cert_abbreviation stripped: '{cert_abbreviation.strip() if cert_abbreviation else ''}'")
    print(f"   cert_abbreviation is truthy: {bool(cert_abbreviation and cert_abbreviation.strip())}")
    ...
    print(f"   Logic result: {'Using abbreviation' if cert_abbreviation else 'Using full name'}")
    ...
    if rename_response.status_code == 200:
        ...
        if expected_cert_identifier == actual_cert_identifier:
            ...
            print(f"   🔍 ANALYSIS: The backend is using different logic than expected")
