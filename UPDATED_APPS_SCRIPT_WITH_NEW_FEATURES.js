/**
 * Ship Management System - Fixed Apps Script v3.7 + Multi Cert Upload Fix
 * With Updated Fallback Structure Matching Backend + RENAME FILE Support + MULTI CERT FIX
 */
...
function handleRequest(e) {
  try {
    ...
    if (e && e.postData && e.postData.contents) {
      ...
      return createJsonResponse(true, "Ship Management Apps Script v3.7 + MULTI CERT FIX is WORKING!", {
        version: "3.7-updated-with-rename-file-and-multi-cert-fix",
        ...
        fixes: [
          ...
          "Updated fallback structure to match backend sidebar (Class & Flag Cert)",
          "Support for parent_category parameter in upload_file_with_folder_creation",
  ...
  try {
    ...
    if (backendApiUrl) {
      ...
      if (dynamicResult.success) {
        ...
        Logger.log("Dynamic structure fetch failed: " + dynamicResult.error);
      ...
      Logger.log("No backend_api_url provided, using updated fallback structure");
      var fallbackResult = getFallbackFolderStructure();
      folderStructure = fallbackResult.structure;
  ...
  try {
    ...
    if (responseCode === 200) {
      ...
      if (result.success && result.structure) {
        ...
        // UPDATED: Log specific folder names to verify they're updated to Class & Flag Cert
...
/**
 * FIXED: Upload file with proper nested folder creation support for Multi Certificate uploads
 * Now supports parent_category parameter from backend
 */
...
function handleDeleteFile(requestData) {
  try {
    ...
    Logger.log("Deleting file " + fileId + " (permanent: " + permanentDelete + ")");
