...
async def get_ship_form_fields_for_extraction() -> dict:
    ...
    try:
        ...
        ship_fields = {
            ...
            "built_year": "Year built/constructed - 4-digit year as number. Look for 'Built Year', 'Year Built', 'Construction Year', 'Date Built', 'Year of Build', 'Built'. Extract only the YEAR portion for backward compatibility.",
            "delivery_date": "Full delivery date of the vessel - complete date when ship was delivered. Look for 'Date of delivery', 'Delivery Date', 'Delivered', 'Date Built', 'Construction Date', or similar delivery-related dates. Return in DD/MM/YYYY format if found. For SUNSHINE 01, this should be 01/08/2006 from 'Date of delivery: AUGUST 01, 2006'.",
...
async def update_certificate(cert_id: str, cert_data: CertificateUpdate, current_user: UserResponse = Depends(check_permission([UserRole.EDITOR, UserRole.MANAGER, UserRole.ADMIN, UserRole.SUPER_ADMIN]))):
    try:
        ...
        # Auto-determine survey type if certificate or ship data changed
        if updated_cert and updated_cert.get('ship_id'):
            ...
            if ship_data:
                ...
                if any(field in update_data for field in survey_affecting_fields):
                    auto_survey_type = determine_survey_type(updated_cert, ship_data)
                    ...
                    if (auto_survey_type != updated_cert.get('next_survey_type') and 
                        ...
                        logger.info(f"Auto-updated survey type for certificate {cert_id}: {auto_survey_type}")
...
async def create_dynamic_ship_folder_structure(gdrive_config: dict, ship_name: str, company_id: str) -> dict:
    ...
    try:
        ...
        backend_api_url = os.environ.get('BACKEND_API_URL', 'https://fleetops-7.preview.emergentagent.com')
...
class EnhancedSurveyTypeDetermination:
    ...
    def analyze_ship_certificates(self, ship_certificates: list) -> dict:
    ...
    def determine_survey_type_enhanced(self, certificate_data: dict, ship_certificates: list, ship_data: dict) -> tuple:
        """
...
def determine_survey_type_enhanced_wrapper(certificate_data: dict, ship_data: dict, ship_certificates: list = None) -> str:
    """
    Wrapper function for enhanced survey type determination to maintain compatibility
    
    Args:
        certificate_data: Certificate information including type, dates, status
        ship_data: Ship information including age, class society, last surveys
        ship_certificates: Optional list of all certificates for the ship
        
    Returns:
        Enhanced survey type string
    """
    try:
        # Initialize enhanced logic
        enhanced_logic = EnhancedSurveyTypeDetermination()
        
        # If ship_certificates not provided, try to fetch them
        if ship_certificates is None:
            ship_id = ship_data.get('id')
            if ship_id:
                try:
                    ship_certificates = list(mongo_db.find_all("certificates", {"ship_id": ship_id}))
                except:
                    ship_certificates = []
            else:
                ship_certificates = []
        ...
        survey_type, reasoning = enhanced_logic.determine_survey_type_enhanced(
            certificate_data, ship_certificates, ship_data
        )
        
        logger.info(f"Enhanced survey type determination: {survey_type} - {reasoning}")
        return survey_type
        
    except Exception as e:
        logger.error(f"Error in enhanced survey type determination: {e}")
        # Fallback to original logic
        return determine_survey_type(certificate_data, ship_data)
