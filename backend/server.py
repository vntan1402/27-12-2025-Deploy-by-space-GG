from fastapi import FastAPI, APIRouter, Depends, HTTPException, UploadFile, File, Form, Body, Query
...
async def analyze_survey_report_file(
    ...
    try:
        ...
        try:
            ...
            # Set empty OCR metadata for exception case
            analysis_result['_ocr_info'] = {
                'ocr_attempted': False,
                'ocr_success': False,
                'ocr_text_merged': False,
                'error': 'Analysis exception occurred'
            }
...
async def analyze_test_report_file(
    ...
    try:
        ...
        # ✅ NEW: Auto-calculate Valid Date if AI extraction failed
        if not analysis_result.get('valid_date'):
            logger.info("🧮 AI did not extract Valid Date - attempting automatic calculation...")
            ...
            try:
                ...
                if test_report_name and issued_date:
                    ...
                    if calculated_valid_date:
                        ...
                        logger.warning("⚠️ Could not auto-calculate Valid Date")
                else:
                    logger.warning(f"⚠️ Missing required fields for Valid Date calculation: test_report_name={bool(test_report_name)}, issued_date={bool(issued_date)}")
...
async def create_dynamic_ship_folder_structure(gdrive_config: dict, ship_name: str, company_id: str) -> dict:
    ...
    try:
        ...
        backend_api_url = os.environ.get('BACKEND_API_URL', 'https://shipman-progress.preview.emergentagent.com')
