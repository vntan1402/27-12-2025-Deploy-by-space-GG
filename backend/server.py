...
@api_router.post("/certificates/move")
async def move_certificates(
    move_data: dict,
    current_user: UserResponse = Depends(check_permission([UserRole.EDITOR, UserRole.MANAGER, UserRole.ADMIN, UserRole.SUPER_ADMIN]))
):
    """Move certificates to different category/folder with database and Google Drive updates"""
    try:
        certificate_ids = move_data.get("certificate_ids", [])
        target_folder_id = move_data.get("target_folder_id")
        target_category = move_data.get("target_category")
        target_folder_path = move_data.get("target_folder_path")
        
        if not certificate_ids or not target_folder_id:
            raise HTTPException(status_code=400, detail="Missing certificate_ids or target_folder_id")
        ...
        for cert_id in certificate_ids:
            try:
                ...
                # Get company info for Google Drive operation
                ...
                # Move file on Google Drive if file ID exists
                google_drive_file_id = certificate.get("google_drive_file_id") or certificate.get("gdrive_file_id")
                if google_drive_file_id and company_id:
                    try:
                        # Get company Google Drive configuration
                        ...
                        if gdrive_config:
                            ...
                            if apps_script_url:
                                ...
                                logger.info(f"📁 Moving file {google_drive_file_id} to folder {target_folder_id} for certificate {cert_id}")
                                ...
                                async with aiohttp.ClientSession() as session:
                                    async with session.post(
                                        ...
                                        if response.status == 200:
                                            result = await response.json()
                                            if result.get("success"):
                                                logger.info(f"✅ Google Drive file moved successfully for certificate {cert_id}")
                                            else:
                                                logger.warning(f"⚠️ Google Drive move failed for certificate {cert_id}: {result.get('message')}")
                                        else:
                                            logger.warning(f"⚠️ Google Drive move HTTP error for certificate {cert_id}: {response.status}")
                    except Exception as gdrive_error:
                        logger.warning(f"⚠️ Google Drive move failed for certificate {cert_id}: {str(gdrive_error)}")
                
                # Update certificate in database
                update_data = {}
                if target_category:
                    update_data["category"] = target_category
                ...
                results.append({
                    "certificate_id": cert_id,
                    "success": True,
                    "google_drive_moved": bool(google_drive_file_id and company_id),
                    "database_updated": bool(update_data)
                })
                
            except Exception as cert_error:
                logger.error(f"Error moving certificate {cert_id}: {str(cert_error)}")
                results.append({
                    "certificate_id": cert_id,
                    "success": False,
        ...
        successful_moves = [r for r in results if r.get("success")]
        failed_moves = [r for r in results if not r.get("success")]
        
        return {
            "success": True,
            "message": f"Processed {len(certificate_ids)} certificates",
            "results": results,
            "summary": {
        ...
        logger.error(f"❌ Error in move certificates operation: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to move certificates: {str(e)}")
