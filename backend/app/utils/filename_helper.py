...
def generate_company_certificate_filename(
    ...
    """
    Generate filename for company certificate with pattern:
    {CompanyName}_{CertAbbreviation}_{CertNo}_{IssueDate_DDMMYYYY}.{ext}
    
    Example: HAI AN CONTAINER_DOC_PM242633_07052024.pdf
    
    Args:
        company_name: Company name
        cert_abbreviation: Certificate abbreviation (DOC, SMC, etc.)
    ...
    try:
        ...
        # 1. Clean company name (KEEP SPACES)
        company_name_clean = _clean_filename_component(company_name or "Unknown Company", keep_spaces=True)
        
        # 2. Clean cert abbreviation (KEEP SPACES)
        cert_abbr_clean = _clean_filename_component(cert_abbreviation or "CERT", keep_spaces=True)
        
        # 3. Clean cert number (KEEP SPACES and special characters like hyphens)
        cert_no_clean = _clean_filename_component(cert_no or "000000", keep_spaces=True) if cert_no else "000000"
        ...
        # 6. Final cleanup: normalize multiple spaces
        filename_base = re.sub(r'\s+', ' ', filename_base)
        
        # 7. Smart truncate if too long (max 150 chars before extension)
        max_length = 150
        if len(filename_base) > max_length:
            logger.warning(f"Filename too long ({len(filename_base)} chars), truncating company name")
            # Truncate company name only to preserve important parts
            other_parts_length = len(cert_abbr_clean) + len(cert_no_clean) + len(date_str) + 3  # 3 underscores
            available_for_company = max_length - other_parts_length
            if available_for_company > 20:  # Keep at least 20 chars for company name
                company_name_clean = company_name_clean[:available_for_company].strip()
            filename_base = "_".join([company_name_clean, cert_abbr_clean, cert_no_clean, date_str])
        
        # 8. Add extension
        new_filename = f"{filename_base}.{file_ext}"
        
        logger.info(f"Generated company certificate filename: {new_filename}")
        return new_filename
        
    except Exception as e:
        logger.error(f"Error generating company certificate filename: {e}")
        # Fallback to timestamp-based name
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        return f"company_cert_{timestamp}.{file_ext}"
