...
{{
  ...
  "cert_abbreviation": "Certificate abbreviation - MUST follow these rules: (1) If cert_name contains 'STATEMENT OF COMPLIANCE', generate abbreviation as 'SOC-XXX' where XXX is the first letters of remaining significant words. Example: 'INTERIM STATEMENT OF COMPLIANCE (SOC) INTERNATIONAL BALLAST WATER MANAGEMENT' → 'SOC-IBWM' (SOC + I from INTERNATIONAL + B from BALLAST + W from WATER + M from MANAGEMENT). (2) For other certificates: Use standard abbreviations (CSSC, LLC, IOPP, etc.) or generate from first letters of significant words.",
  "cert_no": "Certificate number (alphanumeric)",
  "cert_type": "Certificate type - MUST be EXACTLY ONE of these 7 options: 'Full Term', 'Interim', 'Provisional', 'Short term', 'Conditional', 'Statement', 'Other'. ⚠️ CRITICAL RULES: (1) If certificate name/title contains word 'INTERIM' anywhere → ALWAYS use 'Interim' (highest priority, even if name also contains 'Statement'). Example: 'INTERIM STATEMENT OF COMPLIANCE' → 'Interim'. (2) If certificate name contains 'STATEMENT' but NOT 'INTERIM' → use 'Statement'. (3) Otherwise: 'Full Term' (standard 5-year certificate), 'Provisional' (pending full certification), 'Short term' (limited validity), 'Conditional' (with conditions), 'Other' (if none match).",
