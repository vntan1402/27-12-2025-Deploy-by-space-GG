...
class OtherDocumentService:
    ...
    async def create_other_document(doc_data: OtherDocumentCreate, current_user: UserResponse) -> OtherDocumentResponse:
        ...
        check_create_permission(current_user, "other_document", ship_company_id)
        ...
        try:
            ...
            audit_service = OtherDocumentService.get_audit_log_service()
            user_dict = {
                'id': current_user.id,
    ...
    async def update_other_document(doc_id: str, doc_data: OtherDocumentUpdate, current_user: UserResponse) -> OtherDocumentResponse:
        ...
        ship_company_id = ship.get("company")
        check_edit_permission(current_user, "other_document", ship_company_id)
        ...
        # Log audit
        try:
            ship = await mongo_db.find_one("ships", {"id": updated_doc.get("ship_id")})
            ship_name = ship.get("name", "Unknown Ship") if ship else "Unknown Ship"
    ...
    async def delete_other_document(
        ...
        ship_company_id = ship.get("company")
        check_delete_permission(current_user, "other_document", ship_company_id)
    ...
    async def upload_folder_streaming(
        ...
        """
        Upload folder with PARALLEL streaming (optimized).
        - Files upload in parallel with 1s staggered delay
        - Memory efficient: Read file only when needed
        """
        try:
            total_files = len(files)
            logger.info(f"📁 Uploading folder (parallel streaming): {folder_name} with {total_files} files")
            ...
            logger.info("📤 Starting parallel streaming upload to Google Drive...")
            ...
            logger.info("✅ Parallel streaming upload completed")
            logger.info(f"   Folder ID: {folder_id}")
            ...
            logger.error(f"❌ Error uploading folder (parallel streaming): {e}")
            import traceback
            logger.error(traceback.format_exc())
            raise HTTPException(status_code=500, detail=str(e))
