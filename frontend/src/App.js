            ...
            <>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <ol className="text-sm text-green-700 space-y-1">
                  ...
                  <li>3. {language === 'vi' ? 'Thêm Authorized redirect URI:' : 'Add Authorized redirect URI:'} <code>https://shipwise-13.preview.emergentagent.com/oauth2callback</code></li>
...
const AddRecordModal = ({ onClose, onSuccess, language, selectedShip, availableCompanies, ships }) => {
  ...
  // Handle duplicate resolution in AddRecordModal
  const handleDuplicateResolution = async (action) => {
    try {
      if (!duplicateModal || !duplicateModal.show) {
        console.warn('Duplicate modal not initialized');
  ...
  // Handle ship name mismatch resolution in AddRecordModal
  const handleMismatchResolution = async (action) => {
    try {
      if (!mismatchModal || !mismatchModal.show) {
        console.warn('Mismatch modal not initialized');
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          if (fileResult.status === 'success' && fileResult.analysis?.category === 'certificates') {
            // Update status to checking
            setMultiFileUploads(prev => prev.map((item, index) => 
              index === i ? { ...item, status: 'checking', stage: '🔍 Checking for duplicates...', progress: 50 } : item
            ));
            
            // Continue processing even if Google Drive fails
              ...
              <div className="text-center">
                ...
                >
                  ...
                  <input
                    ...
                    multiple
                    className="sr-only"
                    onChange={(e) => selectedShip ? handleMultiFileUpload(e.target.files) : null}
                    accept="*/*"
                ...
                {!selectedShip && (
                  <p className="text-sm text-gray-500 mt-2">
                    {language === 'vi' 
                      ? 'Upload sẽ được kích hoạt sau khi chọn tàu'
                      : 'Upload will be enabled after selecting a ship'
                    }
                  </p>
                )}
              ...
              {multiFileUploads && multiFileUploads.length > 0 && (
                <div className="mt-6 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {!isMultiFileProcessing && multiFileUploads.some(file => file.status === 'completed') && (
                      <button
                        onClick={() => {
                          // Clear upload results and trigger certificate refresh
                          setMultiFileUploads([]);
                          onSuccess('certificate');
                          toast.info(language === 'vi' ? 'Đã làm mới danh sách certificate' : 'Certificate list refreshed');
                        }}
