    ...
    try {
      ...
      if (error.response?.status === 403) {
        const message = error.response?.data?.detail || 
          (language === 'vi' 
            ? 'Bạn không có quyền xem Crew Certificates' 
            : 'You do not have permission to view Crew Certificates');
          ...
          <button 
            onClick={async () => {
              try {
                ...
                toast.success(language === 'vi' ? '✅ Đã làm mới' : '✅ Refreshed', { id: 'refresh' });
              } catch (error) {
                console.error('Error refreshing:', error);
                // Still fetch even if recalculate fails
                await fetchCertificates();
                toast.success(language === 'vi' ? '✅ Đã làm mới' : '✅ Refreshed', { id: 'refresh' });
              }
            }}
                ...
                sortedCertificates.map((cert, index) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-4 whitespace-nowrap border-r border-gray-200">
                      <span 
                        className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full cursor-help ${
