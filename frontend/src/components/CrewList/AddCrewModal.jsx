...
export const AddCrewModal = ({ 
  ...
  const handleBatchProcessing = async (files) => {
    ...
    for (let i = 0; i < validFiles.length; i++) {
      ...
      try {
        const result = await processSingleFileInBatch(file, i + 1);
        results.push(result);
  ...
  const processSingleFileInBatch = async (file, index) => {
    try {
      const shipName = formData.status === 'Standby' ? '-' : (selectedShip?.name || '-');
      ...
      const crewData = {
        ...
        rank: '',
        seamen_book: '',
        status: formData.status === 'Standby' ? 'Standby' : 'Sign on',
        ship_sign_on: formData.status === 'Standby' ? '-' : (selectedShip?.name || '-'),
        place_sign_on: '',
        date_sign_on: '',
        date_sign_off: ''
      };
