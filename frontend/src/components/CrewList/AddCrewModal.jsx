...
export const AddCrewModal = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      ...
      {showInvalidDocModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]" onClick={() => setShowInvalidDocModal(false)}>
            ...
            <div className="flex items-start mb-4">
              ...
              <div className="ml-4 flex-1">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  {language === 'vi' ? 'File không hợp lệ' : 'Invalid File'}
                </h3>
                ...
                {invalidDocDetails.type && invalidDocDetails.type !== 'unknown' && (
                  <p className="text-sm text-gray-500 mt-2">
                    {language === 'vi' ? 'Loại tài liệu phát hiện' : 'Detected document type'}: <span className="font-medium">{invalidDocDetails.type}</span>
                  </p>
                )}
              </div>
            </div>
