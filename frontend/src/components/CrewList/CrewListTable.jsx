...
export const CrewListTable = ({ 
  ...
  const [showBulkEditPlaceSignOff, setShowBulkEditPlaceSignOff] = useState(false);
  const [bulkPlaceSignOn, setBulkPlaceSignOn] = useState('');
  const [bulkPlaceSignOff, setBulkPlaceSignOff] = useState('');
  ...
  const handleBulkUpdateShipSignOn = async () => {
    ...
    try {
      ...
      if (isSignOff) {
        // Sign Off flow: Update DB first, then call API with skip_validation
