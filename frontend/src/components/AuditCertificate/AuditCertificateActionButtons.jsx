/**
 * Audit Certificate Action Buttons Component  
 * Top action buttons for audit certificate list
 */
import React from 'react';

export const AuditCertificateActionButtons = ({
  language,
  selectedShip,
    ...
    <div className="flex justify-between items-center mb-4">
      <div className="flex items-center gap-3">
        <h3 className="text-lg font-semibold text-gray-800">
          {language === 'vi' ? 'Danh mục Audit Certificate' : 'Audit Certificate List'}
        </h3>
      ...
      <div className="flex gap-3">
        {/* Update Next Survey Button - Ẩn cho Viewer */}
        {!isViewer && (
          <button
        ...
        {/* Upcoming Survey Button - Ẩn cho Viewer */}
        {!isViewer && (
          <button
            onClick={onUpcomingSurvey}
            disabled={isLoadingUpcomingSurvey}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              isLoadingUpcomingSurvey 
                ? 'bg-orange-400 cursor-wait' 
                : 'bg-orange-600 hover:bg-orange-700 cursor-pointer'
            } text-white`}
            title={language === 'vi' ? 'Kiểm tra các chứng chỉ sắp đến hạn audit' : 'Check upcoming audit certificates'}
          >
            {isLoadingUpcomingSurvey ? (
              <>
