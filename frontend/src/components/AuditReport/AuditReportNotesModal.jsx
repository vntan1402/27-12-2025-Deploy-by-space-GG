/**
 * Audit Report Notes Modal Component
 * Modal for viewing and editing audit report notes
 */
import React from 'react';

export const AuditReportNotesModal = ({
  isOpen,
  onClose,
  auditReport,
  notes,
          ...
          <div className="flex justify-between items-start">
            <div>
              <h3 className="text-xl font-bold text-gray-800">
                {language === 'vi' ? '📝 Ghi chú Audit Report' : '📝 Audit Report Notes'}
              </h3>
              {auditReport && (
                <p className="text-sm text-gray-600 mt-1">
                  <span className="font-medium">{auditReport.audit_report_name || 'Audit Report'}</span>
                  {auditReport.audit_report_no && ` - ${auditReport.audit_report_no}`}
                </p>
              )}
            </div>
            <button
        ...
        <div className="mb-6">
          ...
          <textarea
            ...
            placeholder={language === 'vi' 
              ? 'Nhập ghi chú cho audit report này...'
              : 'Enter notes for this audit report...'}
          />
