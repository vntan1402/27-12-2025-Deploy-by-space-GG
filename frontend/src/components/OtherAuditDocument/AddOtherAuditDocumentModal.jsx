/**
 * Add Other Audit Document Modal
...
const AddOtherAuditDocumentModal = ({ 
  ...
  const uploadInBackground = async (documentId, shipId, file) => {
    ...
    try {
      const result = await otherAuditDocumentService.uploadFileForDocument(
        documentId,
        shipId,
        file
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (files.length === 0) {
        ...
        await otherAuditDocumentService.create({
        ...
        for (const file of files) {
          ...
          const doc = await otherAuditDocumentService.create({
            ship_id: selectedShip.id,
  ...
  if (!show && !showFloatingProgress) return null;
      ...
      <div className="bg-white rounded-xl shadow-2xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-bold text-gray-800">
            {language === 'vi' ? '📁 Thêm Tài liệu Audit' : '📁 Add Other Audit Document'}
          </h3>
          <button
            onClick={onClose}
...
export default AddOtherAuditDocumentModal;
