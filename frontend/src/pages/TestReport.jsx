...
const TestReport = () => {
  ...
  const [selectedSubMenu, setSelectedSubMenu] = useState('test_report');
  ...
  const fetchShips = async (forceRefresh = false) => {
    try {
      ...
      if (!Array.isArray(data)) {
        ...
        setShips(data);
        console.log('✅ [TestReport] Ships loaded:', data.length);
      }
    } catch (error) {
  ...
  const handleSubMenuChange = (submenuKey) => {
    ...
    if (routes[submenuKey] && routes[submenuKey] !== '/test-report') {
