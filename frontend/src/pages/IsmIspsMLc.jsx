...
const IsmIspsMLc = () => {
  ...
  const getFilteredCertificates = () => {
    ...
    filtered.sort((a, b) => {
      ...
      if (['issue_date', 'valid_date', 'last_endorse'].includes(certificateSort.column)) {
        return compareDates(a[certificateSort.column], b[certificateSort.column]) * (certificateSort.direction === 'asc' ? 1 : -1);
      }
  ...
  const getCertificateStatus = (cert) => {
    const dueSoonDays = 90; // Audit certificates use 90 days
    ...
    if (cert.valid_date.includes('/')) {
      const parts = cert.valid_date.split('/');
      if (parts.length === 3) {
  ...
  // Bulk Auto Rename Files - BACKGROUND VERSION
  ...
  return (
    ...
    >
      ...
      <ShipSelectionModal
        ...
        title={language === 'vi' ? 'Chọn tàu để xem thông tin chứng chỉ' : 'Select a ship to view certificate information'}
      />
