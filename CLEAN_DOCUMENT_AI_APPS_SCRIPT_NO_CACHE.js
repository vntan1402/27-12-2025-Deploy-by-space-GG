 ...
 * Version: Clean (No Cache Busting) - Working Version
...
function handleRequest(e) {
  try {
    ...
    if (e && e.postData && e.postData.contents) {
      ...
      return createJsonResponse(true, "Document AI Service (Clean) is WORKING!", {
        version: "clean-no-cache",
...
// CLEAN: Handle passport analysis without cache busting
function handleAnalyzePassport(data) {
  try {
    ...
    console.log("🔄 PASSPORT ANALYSIS - Clean Version");
    ...
    // CLEAN request payload - no cache busting labels or metadata
    var requestPayload = {
      rawDocument: {
        content: fileContent,
    ...
    // CLEAN headers - no cache busting headers
    var options = {
      method: "POST",
      headers: {
        Authorization: "Bearer " + accessToken,
    ...
    console.log("📡 Sending clean request to Document AI API");
...
function createJsonResponse(success, message, data) {
  var result = {
    ...
    message: message,
    timestamp: new Date().toISOString(),
    service: "Document AI Clean v1.0"
  };
