/**
 * Ship Management System - Dynamic Apps Script v3.4
 * With Dynamic Folder Structure from Homepage Sidebar
 */
...
function handleRequest(e) {
  try {
    ...
    if (e && e.postData && e.postData.contents) {
      ...
      return createJsonResponse(true, "Ship Management Apps Script v3.4 is WORKING!", {
        version: "3.4-dynamic-sidebar-structure",
        ...
        features: [
          ...
          "Fallback to cached structure if API fails",
...
/**
 * Get dynamic folder structure from Homepage Sidebar
 * @param {string} backendApiUrl - The backend API URL to fetch structure
 * @returns {Object} - Folder structure object
 */
function getDynamicFolderStructure(backendApiUrl) {
  try {
    // Construct API URL for sidebar structure
    var apiUrl = backendApiUrl + "/api/sidebar-structure";
    
    Logger.log("📡 Fetching dynamic folder structure from: " + apiUrl);
    
    // Make API call to get current sidebar structure
    ...
    if (responseCode === 200) {
      ...
      if (result.success && result.structure) {
        ...
        Logger.log("📊 Subcategories: " + result.metadata.total_subcategories);
      ...
      return { success: false, error: "API call failed: " + responseCode };
    }
...
/**
 * Get fallback folder structure (cached/default)
 * @returns {Object} - Fallback folder structure
 */
function getFallbackFolderStructure() {
  Logger.log("📋 Using fallback folder structure");
  
  // This is the most recent known structure as fallback
...
function handleCreateCompleteShipStructure(requestData) {
  try {
    ...
    var backendApiUrl = requestData.backend_api_url; // New parameter for API URL
    ...
    if (backendApiUrl) {
      ...
      if (dynamicResult.success) {
        ...
        Logger.log("🔄 Falling back to cached structure...");
        var fallbackResult = getFallbackFolderStructure();
        folderStructure = fallbackResult.structure;
        structureSource = "fallback_cached";
        structureMetadata.fallback_reason = dynamicResult.error;
      }
    } else {
      Logger.log("⚠️ No backend_api_url provided, using fallback structure");
      var fallbackResult = getFallbackFolderStructure();
      folderStructure = fallbackResult.structure;
...
function handleUploadFileWithFolderCreation(requestData) {
  try {
    ...
    // Categories under Document Portfolio (dynamic list could be used here too)
