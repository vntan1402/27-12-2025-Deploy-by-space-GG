...
def test_with_real_ship():
    ...
    # Login
    login_response = requests.post('https://shipmanage.preview.emergentagent.com/api/auth/login', 
                                  json={'username': 'admin', 'password': 'admin123'})
    token = login_response.json()['access_token']
    headers = {'Authorization': f'Bearer {token}'}
    
    # Get existing ships
    ships_response = requests.get('https://shipmanage.preview.emergentagent.com/api/ships', headers=headers)
...
"""
    ...
    upload_response = requests.post(
        'https://shipmanage.preview.emergentagent.com/api/certificates/upload-multi-files',
        files=files,
        headers={'Authorization': f'Bearer {token}'},
        timeout=120
    )
