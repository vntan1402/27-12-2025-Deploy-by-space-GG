...
async def test_ai_direct():
    ...
    response = requests.post('https://shipdata-hub.preview.emergentagent.com/api/auth/login', json=login_data)
    token = response.json()['access_token']
    headers = {'Authorization': f'Bearer {token}'}

    # Get certificate
    cert_response = requests.get('https://shipdata-hub.preview.emergentagent.com/api/certificates?ship_id=vesseldocs', headers=headers)
